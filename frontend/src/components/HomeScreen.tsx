...
import type { RouteData, VillaStreet, Neighborhood } from "../types";
        ...
        <div className="bg-slate-800 rounded-2xl p-4 mb-4">
          ...
          >
            <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">
              Villa kvarterer ({villaStreets.length})
            </h2>
          ...
          {dataLoading && (
            <p className="text-xs text-slate-500 mt-2">Finder villa kvarterer...</p>
          )}
          ...
          {showVillas && (
            <div className="mt-3 max-h-80 overflow-y-auto space-y-1">
              {neighborhoods.map((n, i) => (
                <button
                  key={`n-${i}`}
                  onClick={() => openInGoogleMaps(n.lat, n.lng, n.name)}
                  className="w-full flex items-center justify-between bg-slate-700/50 rounded-lg px-3 py-2 hover:bg-slate-700 transition-colors text-left"
                >
                  <div>
                    <span className="text-sm">{n.name}</span>
                    <span className="text-xs text-slate-400 block">{n.address}</span>
                  </div>
              ...
              {villaStreets.map((s) => (
                <button
                  key={`s-${s.id}`}
                  onClick={() => openInGoogleMaps(s.lat, s.lng, s.name)}
                  className="w-full flex items-center justify-between bg-slate-700/50 rounded-lg px-3 py-2 hover:bg-slate-700 transition-colors text-left"
                >
        ...
        <div className="bg-slate-800 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="flex items-center gap-2 col-span-2">
              ...
              <span className="text-slate-300">Hastighedsgrænse (farve efter hastighed)</span>
            </div>
          </div>
        </div>
      </div>
